    ...
    <div class="bg_box">
      <div class="jb_news_box box_list">
        ...
        <div class="item_box">
          ...
          <div class="item_right">
            <el-select v-model="fatherTarget" placeholder="请选择" style="width: 800px;" @change="fatherTargetChange">
              <el-option v-for="item in targetOptions" :key="item.id" :label="item.targetName" :value="item.id">
              </el-option>
            </el-select>
          </div>
        </div>
        ...
        <div class="item_box">
          ...
          <div class="item_right">
            <el-select v-model="jdName" multiple filterable remote placeholder="请输入姓名或员工号" :remote-method="remoteMethod" @change="select_change" style="width: 800px;">
              <el-option v-for="item in cy_jd_Options" :key="item.user_id" :label="item.checkUserName" :value="item.user_id">
              </el-option>
            </el-select>
          </div>
        </div>
        <div class="item_box">
          <div class="item_left">
            参与人</div>
          <div class="item_right">
            <el-select v-model="cyName" multiple filterable remote placeholder="请输入姓名或员工号" :remote-method="remoteMethod" @change="select_change" style="width: 800px;">
              <el-option v-for="item in cy_jd_Options" :key="item.user_id" :label="item.checkUserName" :value="item.user_id">
              </el-option>
            </el-select>
          </div>
      ...
      <div class="shfs_box box_list" style="padding-bottom: 20px;">
        ...
        <div class="item_box">
          ...
          <div v-if="shType - 0 === 3">
            <el-select v-model="checkUserId" filterable remote clearable placeholder="请输入姓名或工号" :remote-method="remoteMethod_check" @change="select_change" @focus="checkFocus" style="margin: 0 50px;width: 300px;">
...
export default {
  ...
  mounted() {
    ...
    this.$ajaxMore(
      ...
      res => {
        if (res.success) {
          self.targetOptions = res.data.data;
        } else {
          self.$message({
            message: res.msg,
            type: "error"
    ...
    this.remoteMethod_check(this.$store.state.nickname);
    this.getDeptAndRole();
  },
  methods: {
    formChange(str, val) {
    ...
    remoteMethod(query) {
      if (query !== "") {
        ...
        this.$ajaxMore(
          ...
          res => {
            if (res.success) {
              this.cy_jd_Options = res.data.data;
            } else {
              this.$message({
                type: "error",
                message: res.msg
    ...
    doSubmit() {
      ...
      */
      ...
      this.$ajaxMore(
        "post",
        this.HOST + "/tr/trPlan/web/addPlan",
        data,
        res => {
          if (res.success) {
            this.$message({
