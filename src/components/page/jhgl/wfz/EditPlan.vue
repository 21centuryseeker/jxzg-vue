<template>
  <div class="scroll_x_hidden_box">
    <!-- <div class="container_box_header">编辑计划</div> -->
    <!-- 基本信息 -->
    <div class="bg_box">
      <div class="jb_news_box box_list">
        ...
        <div class="item_box">
          <div class="item_left">
            <span class="need">*</span>所属目标</div>
          <div class="item_right">
            <el-select v-model="fatherTarget" placeholder="请选择" style="width: 800px;" @change="fatherTargetChange">
              <el-option v-for="item in targetOptions" :key="item.id" :label="item.target_name" :value="item.id">
              </el-option>
            </el-select>
          </div>
        </div>
        ...
        <div class="item_box">
          ...
          <div class="item_right">
            <el-select
              ...
              :remote-method="remoteMethod_jdName"
              @change="select_change"
              style="width: 800px;">
              <el-option
                v-for="item in jdrOptions"
                :key="item.user_id || item.supervisors"
                :label="item.checkUserName || item.suv_names"
                :value="item.user_id || item.supervisors" >
              </el-option>
            </el-select>
          </div>
        </div>
        <div class="item_box">
          ...
          <div class="item_right">
            <el-select
              ...
              remote
              placeholder="请输入姓名或员工号"
              :remote-method="remoteMethod_cyName"
              @change="select_change"
              style="width: 800px;">
              <el-option
                v-for="item in cyrOptions"
                :key="item.user_id || item.joins"
                :label="item.checkUserName || item.join_names"
                :value="item.user_id || item.joins">
              </el-option>
            </el-select>
          </div>
        </div>
...
export default {
  ...
  mounted () {
    ...
    self.$ajaxMore(
      'post',
      self.HOST + '/tr/trPlan/web/getPlanDetail',
      { planId: self.$router.currentRoute.params.id },
      (res) => {
        if (res.success) {
          this.deptId = res.obj.dept_id
          self.fatherTarget = res.obj.target_id
          self.planName = res.obj.plan_name
          self.targetInfo = res.obj.plan_content
          self.targetTime = [res.obj.start_time, res.obj.end_time]
          self.cyName = res.obj.joinIds
          self.jdName = res.obj.supervisorIds
          // if (res.obj.fileListForAdd.length) {
          //   self.fileListOld = res.obj.fileListForAdd
          //   for (let item of res.obj.fileListForAdd) {
          //     self.fileList.push({
          ...
          //   }
          // }
          self.fileList = res.obj.fileListForAdd
          self.cyrOptions = res.obj.joinList
          self.jdrOptions = res.obj.supervisorList
          if (res.obj.indexId) {
            self.unit = res.obj.unit
            self.standDesc = res.obj.standDesc
            self.zbName = res.obj.indexName
            self.zb_id = res.obj.indexId
            self.khbz = res.obj.planStandContion
            self.khbz_input = res.obj.planStand
            self.targetStandInfo = res.obj.planStandInfo
            self.yjLists = []
            if (res.obj.warn.length) {
              for (let item of res.obj.warn) {
                self.radio = item.warningType.toString()
                self.yjTime = self.mydateFormat.dateCodeYmd(
                  item.waringTime
                )
                self.yjLists.push({
          ...
          self.shType = res.obj.checkTypeId
          if (res.obj.checkDeptId && res.obj.checkTypeId - 0 === 3) {
            self.checkUserId = res.obj.checkDeptId
            self.sgrOptions.push({
              user_id: res.obj.check_user_id + '_' + res.obj.check_user_dept_id,
              checkUserName: res.obj.check_user_name + '(' + res.obj.check_user_id + ',' + res.obj.dept_name + ')',
  ...
  methods: {
    ...
    doSubmit () {
      ...
      if (self.zb_id) {
        ...
        self.$ajaxMore(
          'post',
          self.HOST + '/tr/trPlan/web/editPlan',
          data,
          (res) => {
            if (res.success) {
              self.$message({
