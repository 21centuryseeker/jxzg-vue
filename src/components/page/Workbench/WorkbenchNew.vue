  ...
  <div id="workbench">
    ...
    <div class="mainContentB paddingTop66">
      <div class="tab-content">
        ...
        <el-dialog title="通知" :visible.sync="dialogVisible" width="30%" @close="close1">
          <span>{{contentA}}</span>
                  ...
                  <div class="contentX">
                    <el-collapse accordion @change="accordionFn">
                      <template v-for="(item,index) in gatherData">
                        <el-card class="box-card" :class="flagAdd==item.id?'activeCardStyle':''" :key="index">
                                  ...
                                  <li class="firstL">
                                    <div class="text">
                                      <div class="nameTarge">
                                        ...
                                        <a href="javascript:void(0)" class="goTablelink goTablelinkA" :class="flagAdd==item.id?'bgGreen':''">
                                          {{item.target_name}}
                                        </a>
                                        <!-- </span>  -->
                                      </div>

                                      <div class="group groupA cleatfix">
                                        <div class="prgross">
                              ...
                              <div class="targerLIstA" v-if="targerLIstFlag">
                                <div class=" ">
                                  <template v-for="(item,index) in decomposeData">
                                    <div :key="index">
                                      <div class="targerLIst">
                                        <ul class="clearfix" style="cursor: pointer" @click="zTarger(item.id)">
                                          <li class="firstL">
                                            <div class="text">
                                              <!-- <span style="padding-left:10px">
                                                                  <i class="iconfont">&#xe6c9;</i>
                                              ...
                                              <div class="nameTarge">
                                                ...
                                                <a href="javascript:void(0)" class="goTablelink goTablelinkA">
                                                  子目标{{index+1}}: {{item.target_name}}
                                                </a>
    ...
    <el-dialog title="本月进度填报" :visible.sync="dialogVisible_pl" width="30%">
      <div style="font-size: 16px;margin-bottom: 20px;padding-left: 20px;">{{theDetailName}}</div>
...
export default {
  ...
  methods: {
    ...
    targetNoticeFn(deptId) {
      ...
      self.$ajaxMore(
        "post",
        self.HOST + "/tr/trUser/web/getUserMessageInfo",
        newData,
        res => {
          if (res.success) {
            if (res.obj.length == 0) {
    ...
    close(v) {
      ...
      self.$ajax(
        "post",
        self.HOST + "/tr/trUser/web/delBatchMessage",
        { list: id },
        res => {
          if (res.success) {
    ...
    toDetaiLink(v) {
      ...
      self.$ajax(
        "post",
        self.HOST + "/tr/trUser/web/editUserMessageInfo?id=" + self.Id,
        {},
        res => {
          if (res.success) {
            self.messageFn();
    ...
    targetFn(deptId) {
      ...
      self.$ajax(
        "post",
        self.HOST + "/tr/workbench/web/getWorkbenchData",
        { deptId: deptId },
        res => {
          if (res.success) {
            // 目标总数
            ...
            for (
              var i = 0, len = res.data.targetComplate.wfz.length;
              i < len;
              i++
            ) {
            ...
            self.planNum = 0;
            for (
              var i = 0, len = res.data.planComplate.wfz.length;
              i < len;
              i++
            ) {
            ...
            self.planShNum = 0;
            for (
              var i = 0, len = res.data.planComplate.wsh.length;
              i < len;
              i++
            ) {
            ...
            self.planJdNum = 0;
            for (
              var i = 0, len = res.data.planComplate.wjd.length;
              i < len;
              i++
            ) {
    ...
    chartBoxFn() {
      ...
      var myChart = this.$echarts.init(
        document.getElementsByClassName("chartBox")[0]
      );
    ...
    planAudit(id) {
      ...
      this.$ajaxMore(
        "post",
        this.HOST + "/tr/planAudit/web/sendMessage",
        data,
        res => {
          if (res.success) {
            this.$message({
...
.planargeTitle .operate,
.targerLIst li.operate {
  width: 20%;
}
.planargeTitle .targetCM,
.targerLIst li.targetCM {
  width: 10%;
}
.planargeTitle .last,
.targerLIst li.last {
...
.planTbableList .el-table .cell,
.planTbableList .el-table th.monthCell div {
...
#workbench .planargeA .targerLIst .firstL .goTablelinkA {
  top: -4px;
}
.mainContentB .groupA {
  bottom: 9px;
