...
export default {
  ...
  methods: {
    ...
    processTeacher: function (val) {
      this.baseAjax(46, '2017各专业部毕业生情况').then((res) => { this.graphByLine(res, 'chart_zsqk', 'title1') })
      this.baseAjax(41, '当前学年学生流失情况').then((res) => { this.graphByBing(res, 'chart_2', 'title2') })
      this.baseAjax(42, '当前学年学生流失情况').then((res) => { this.graphByBing(res, 'chart_3', 'title3') })
      this.baseAjax(50, '当前学年学生流失情况').then((res) => {
        res.data.unit = '%'
        this.graphByLine(res, 'chart_sydfx', 'title4')
      })
      this.baseAjax(44, '当前学年学生流失情况').then((res) => { this.graphByBing(res, 'chart_bysjy', 'title5') })
      this.baseAjax(45, '2017各专业部毕业生情况').then((res) => { this.graphByLine(res, 'chart_bysjy1', 'title6') })
      this.baseAjax(43, '2017各专业部毕业生情况').then((res) => {
        this.$set(this.titleMap, 'title7', res.data.title)
        let series = []
      ...
      this.baseAjax(40, '学生人数').then(this.graphByTeacher)
    },
    graphByTeacher: (res) => {
      this.value_xsrs_list = res.data.value1 + ''

      this.c2 = res.data.value6 + ':1'

      this.c3 = res.data.value7
    ...
    switch: function (val) {
      switch (val) {
        case 4: // 教师画像
          this.processTeacher(val)
          this.bigTitle = '在职教职工总数'
          this.title1 = '师生比'
          this.title2 = '教学事故'
          break
      }
