...
export default {
  ...
  methods: {
    processClass: function (val) {
      this.baseAjax(34, '2017各专业部毕业生情况11').then((res) => { this.graphByBing(res, 'chart_zsqk', 'title1') })
      this.baseAjax(36, '学生违纪专业分布').then((res) => { this.graphByBing(res, 'chart_bysjy', 'title5') })
      this.baseAjax(33, '学生人数').then(this.graphByClass)
    },

    graphByClass: (res) => {
      this.value_xsrs_list = res.data.value1 + ''
      this.c2 = res.data.value2 + '门'
      this.c3 = res.data.value3 + '门'
      this.sutil = '人'
    },
