    ...
    <div class="mianWrapper clearfix">
      ...
      <div class="MiddlePart  floatL">
        <div class="MiddlePart_inner" >
          ...
          <!--</div>-->
          <div style="margin-top: 10%" class="item" :class="{'item1':isShow ==2}">
            <h5 class="title">{{titleMap.title4}}</h5>
            <div id="chart_sydfx" class="chart_zsqk"></div>
          </div>
          <div style="margin-top: 20%"  class="item" :class="{'item1':isShow ==2}">
            <h5 class="title">{{titleMap.title2}}</h5>
            <div id="chart_5" class="chart_zsqk"></div>
          </div>
...
export default {
  ...
  methods: {
    ...
    processSchool: function (val) {
      this.baseAjax(5, '2017各专业部毕业生情况11').then((res) => { this.graphByLine(res, 'chart_zsqk', 'title1') })
      this.baseAjax(6, '2017各专业部毕业生情况').then((res) => {
        this.$set(this.titleMap, 'title2', res.data.title)
        let series = []
        ...
        // console.log('1111111111111', res.data.xAxis)
        this.$ChartUtil.drawChartShow(
          'chart_2',
          '',
          res.data.legend,
        ...
        // this.graphByLine(res, 'chart_bysjy1', 'title6') })
      })
      this.baseAjax(24, '2017各专业部毕业生情况').then((res) => {
        this.$set(this.titleMap, 'title2', res.data.title)
        let series = []
        for (let key in res.data.series) {
          series.push({
      ...
      this.baseAjax(7, '当前学年学生流失情况').then((res) => { this.graphByBing(res, 'chart_3', 'title3') })
      this.baseAjax(21, '当前学年学生流失情况').then((res) => { this.graphByBing(res, 'chart_sydfx', 'title4') })
      this.baseAjax(64, '当前学年学生流失情况').then((res) => { this.graphByBing(res, 'chart_bysjy', 'title5') })
      this.baseAjax(16, '2017各专业部毕业生情况').then((res) => {
        this.$set(this.titleMap, 'title6', res.data.title)
        let series = []
        for (let key in res.data.series) {
          series.push({
            type: 'bar',
      ...
      this.baseAjax(11, '当前学年学生流失情况').then((res) => { this.graphByBing(res, 'chart_bysjy2', 'title7') })
      this.baseAjax(1, '学生人数').then(this.graphBySchool)
    },

    graphBySchool: (res) => {
      this.value_xsrs_list = res.data.value5 + ''

      this.c2 = res.data.value3 + '个'

      this.c3 = res.data.value4 + '个'
